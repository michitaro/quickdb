...
def run(make_env, context={}, time=None):
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            if auth_line.startswith('ng:'):
                ...
                request = dict(worker=worker, make_env=make_env, context=context)
                serialize.dump(request, wfile)
                response = serialize.load(rfile)
