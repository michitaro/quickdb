...
from quickdb.sql2mapreduce.sqlast.sqlast import (Context, Expression, Row,
                                                 SqlError)
...
class HistogramAggCall(AggCall):
    ...
    def subaggrs(self, ) -> List[AggCall]:
        if self._range is None:
            self._minmax = MinMaxAggCall([self._array], {})
        return [self._minmax]

    def mapper(self, context: Context):
        bins: int = 50 if self._bins is None else self._bins.evaluate(context)
        if self._range is not None:
            row: Row = self._range.evaluate(context)
            if not isinstance(row, Row):
                raise SqlError(f'range must be a tuple: {row}')
            range = row.args
        else:
            range = self._minmax.result
